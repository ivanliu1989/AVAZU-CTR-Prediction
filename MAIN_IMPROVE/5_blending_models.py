...
pred_file4 = 'pred/submission_vw_poly.csv' 
...
start = datetime.now()
with open('Blendling_lg_gbt_fm_poly.csv',"wb") as outfile:
    ...
    for t, row in enumerate(DictReader(open(pred_file1))):
        ...
        click = 0.35*float(click) + 0.3*float(pred_2[ID]) + 0.2*float(pred_3[ID]) + 0.15*float(pred_4[ID])
