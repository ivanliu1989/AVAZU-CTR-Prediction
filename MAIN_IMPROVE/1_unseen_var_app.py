...
with open('data/test_df_app_smooth_ex.csv',"wb") as outfile:
    ...
    for t, row in enumerate(DictReader(open(test_app_path))):
        ...
        if str(app_id) not in app_id_train:
            app_id = -2
        if str(app_domain) not in app_domain_train:
            app_domain = -2
        if str(app_category) not in app_category_train:
            app_category = -2
...
with open('data/train_df_app_smooth_ex.csv',"wb") as outfile:
    ...
    for t, row in enumerate(DictReader(open(train_app_path))):
        ...
        if str(app_id) not in app_id_test:
            app_id = -2
        if str(app_domain) not in app_domain_test:
            app_domain = -2
        if str(app_category) not in app_category_test:
            app_category = -2
