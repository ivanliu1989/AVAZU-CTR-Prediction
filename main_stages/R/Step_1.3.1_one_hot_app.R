...
save_mt(one_hot_1, T, 'hour', train_id_click, test_id);rm(one_hot_1);gc()

one_hot_2 <- model.matrix(~ -1 + C1, train[1], verbose=T);gc();train <- train[-1]
save_mt(one_hot_1, F, 'hour', train_id_click, test_id);rm(one_hot_1);gc()

one_hot_3 <- model.matrix(~ -1 + banner_pos, train[1], verbose=T);gc();train <- train[-1]
save_mt(one_hot_1, F, 'hour', train_id_click, test_id);rm(one_hot_1);gc()

# one_hot_4 <- model.matrix(~ -1 + app_id, train[1], verbose=T);gc();
train <- train[-1]
...
# one_hot_7 <- model.matrix(~ -1 + device_id, train[1], verbose=T);gc();
train <- train[-1]

# one_hot_8 <- model.matrix(~ -1 + device_ip, train[1], verbose=T);gc();
train <- train[-1]
...
### func ###
save_mt <- function(train_mt, id_click, path, train_id_click, test_id){
    path_train <- paste('data/onehot_train_app_',path,'.csv',sep='')
    path_test <- paste('data/onehot_train_app_',path,'.csv',sep='')
    test_mt <- train_mt[(nrow(train_id_click)+1):nrow(train_mt),]
    train_mt <- train_mt[1:nrow(train_id_click),]
    if(id_click==T){
        train_mt <- cbind(train_id_click, train_mt)
        test_mt <- cbind(test_id, test_mt)
        write.csv(train_mt, file = path_train, quote = F, row.names = F)
        write.csv(test_mt, file = path_test, quote = F, row.names = F)
    }else{
        write.csv(train_mt, file = path_train, quote = F, row.names = F)
        write.csv(test_mt, file = path_test, quote = F, row.names = F)
    }
