...
pred_file1 = 'pred/blending/last_0.3928879.csv'
pred_file2 = 'pred/blending/submit_libFM_pred_0.4027643.csv'  
pred_file3 = 'pred/blending/submission_vw_cubic2_0.3957190.csv'  
pred_file4 = 'pred/blending/submit_xgboost_900_03_0.3931970.csv' 
pred_file5 = 'pred/blending/submission_vw_cubic2_0.3957190.csv'   
...
start = datetime.now()
with open('Blending_models_last_5_a.csv',"wb") as outfile:
    ...
    for t, row in enumerate(DictReader(open(pred_file1))):
        ...
        click = (float(click)+float(pred_3[ID])+float(pred_2[ID])+float(pred_4[ID])+float(pred_5[ID]))/5
